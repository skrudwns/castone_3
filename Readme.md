...
주요 에이전트는 PlannerAgent와 EditorAgent로 구성되어 있으며, RAG 기술과 외부 API를 결합하여 사용자에게 최적화된 개인화 여행 일정을 생성해 줍니다.

2. 핵심 대화 흐름
대화는 크게 계획 단계와 편집 단계로 구분되어 진행됩니다.

초기 계획

PlannerAgent가 사용자의 입력을 바탕으로 여행 계획을 수립합니다.

이때 시스템에 사전에 정의된 기계적인 고정 스케줄 규칙을 따릅니다.

일정 생성

각 일차별로 정의된 슬롯을 채우기 위해 find_and_select_best_place 도구를 호출하여 최적의 장소를 검색합니다.
...
특정 장소를 삭제하거나 교체하고, 변경된 동선에 맞춰 시간을 재계산합니다.
...
3. 주요 기능 상세
스케줄링 로직
엄격한 슬롯 할당 에이전트는 자유롭게 일정을 짜는 것이 아니라, 사전에 정의된 슬롯을 채우는 방식으로 작동합니다.

Day 1: 점심 → 카페 → 관광지 → 저녁 (총 4곳)

중간 날: 관광지 → 점심 → 카페 → 관광지 → 저녁 (총 5곳)
...
검색 및 추천 로직
find_and_select_best_place 도구는 정확도를 위해 2단계 검색 전략을 수행합니다.

선호 반영 검색: 사용자 취향을 쿼리에 포함하여 검색합니다.

Fallback 검색: 1단계 결과가 없을 경우, 취향 정보를 제거하고 검색한 뒤 기준점에서 가장 가까운 거리순으로 정렬하여 추천합니다.
...
RAG 데이터 업데이트 로직
사용자가 작성한 리뷰는 서버에 임시 파일로 누적됩니다.

자동 업데이트 트리거: 누적 리뷰가 10개 이상이 되면 업데이트 프로세스가 자동으로 실행됩니다.

지능형 주소 채우기: 새 리뷰에 주소 정보가 누락된 경우, 기존 벡터 DB에서 동일한 장소명을 검색하여 기존에 저장된 상세 주소를 자동으로 가져와 채워넣습니다.

임베딩 모델: 한국어 처리에 특화된 upskyy/bge-m3-korean 모델을 사용하여 데이터를 벡터화합니다.

PDF 생성 로직
사용자가 "PDF 만들어줘"라고 요청하거나 일정을 확정하면 confirm_and_download_pdf 도구가 호출됩니다. 이 도구가 실행되면 상태 변수가 변경되며, UI에 PDF 다운로드 버튼이 렌더링되고 에이전트의 작업이 종료됩니다.
